export default {
    ADD_CONTACT_FORM: 'addContactForm',          // 입력 폼 나타내기
    CANCEL_FORM: 'cancelForm',                   // 입력, 수정 폼 닫기
    EDIT_CONTACT_FORM: 'editContactForm',        // 수정 폼 나타내기
    EDIT_PHOTO_FORM: 'editPhotoForm',            // 사진 편집 폼 나타내기
    FETCH_CONTACTS: 'fetchContacts',             // 연락처 조회

    // 액션에서만 사용
    ADD_CONTACT: 'addContact',                   // 연락처 추가
    UPDATE_CONTACT: 'updateContact',             // 연락처 수정
    UPDATE_PHOTO: 'updatePhoto',                 // 사진 수정
    DELETE_CONTACT: 'deleteContact'              // 연락처 삭제
}